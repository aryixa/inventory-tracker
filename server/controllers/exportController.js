...
// @desc    Export category usage data to CSV
// @route   GET /api/export/category-usage
// @access  Private/Admin
export const exportCategoryUsage = async (req, res) => {
  try {
    const { start_date, end_date } = req.query;
    ...
    const csvData = categoryUsage.map(usage => ({
      category: usage.category,
      thicknessMm: formatDecimal128(usage.thicknessMm),
      totalItemsUsed: usage.totalItemsUsed,
      totalSqmArea: usage.totalSqmArea.toFixed(2)
    }));

    const fileName = `category-usage-export-${Date.now()}.csv`;
    const filePath = path.join(__dirname, '../exports', fileName);
    ...
    const csvWriter = createCsvWriter.createObjectCsvWriter({
      ...
      header: [
        { id: 'category', title: 'Category' },
        { id: 'thicknessMm', title: 'Thickness (mm)' },
        { id: 'totalItemsUsed', title: 'Total Items Used' },
        { id: 'totalSqmArea', title: 'Total Area (sqm)' }
      ]
    });
    ...
    console.error('Export category usage error:', error);
    res.status(500).json({ success: false, message: 'Server error while exporting category usage' });
  }
};
