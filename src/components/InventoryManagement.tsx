...
  // Local match used to filter real-time updates against the current searchTerm
...
    // Convert numeric canon fields to string for fuzzy includes
...
        const msg = error?.response?.data?.message || error?.message || 'Failed to load inventory items';
        toast.error(msg);
      } finally {
        setIsLoading(false);
...
  const handleUseStock = async (quantity: number, reductionReason: 'usage' | 'breakage') => {
    if (!selectedItem || !user) return;
    if (!canReduce) {
...
          {items.map((item) => (
            <div key={item._id} className="bg-blue-50 rounded-lg shadow-md p-6 hover:shadow-lg transition-shadow">
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div className="min-w-0 flex-1">
                    <h3 className="font-bold text-xl text-blue-900 truncate">{item.type}</h3>
                    <p className="text-sm text-gray-900 truncate">{item.brand}</p>
                  </div>
                </div>
              </div>
...
    <span className="text-sm font-medium text-gray-900">Thickness:</span>
    <span className="text-sm font-medium text-gray-900">{item.thicknessMm} mm</span>
  </div>

  <div className="flex justify-between">
  <span className="text-sm font-medium text-gray-900">Length:</span>
  <span className="text-sm font-medium text-gray-900">{item.sheetLengthMm} mm</span>
</div>

<div className="flex justify-between">
  <span className="text-sm font-medium text-gray-900">Width:</span>
  <span className="text-sm font-medium text-gray-900">{item.sheetWidthMm} mm</span>
</div>

  {typeof item.totalSqm === 'number' && (
  <div className="flex justify-between">
    <span className="text-sm font-medium text-gray-900">Total area:</span>
